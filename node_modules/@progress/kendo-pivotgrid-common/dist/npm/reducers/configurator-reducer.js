...
})(PIVOT_CONFIGURATOR_ACTION = exports.PIVOT_CONFIGURATOR_ACTION || (exports.PIVOT_CONFIGURATOR_ACTION = {}));
/**
 * @hidden
 */
exports.configuratorReducer = function (state, action) {
    var newRows;
    var newColumns;
    var newMeasures;
    var newSort;
    ...
    switch (action.type) {
        case PIVOT_CONFIGURATOR_ACTION.toggleSelection: {
            if (Array.isArray(action.payload)) {
                ...
                if (payload_1.type === 2 || 'aggregator' in payload_1) {
                    if (state.measureAxes.some(function (s) { return utils_1.compareAxisWithField(s, payload_1); })) {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.removeMeasureAxis }));
                    }
                    else {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.addMeasureAxis }));
                    }
                }
                else if (payload_1.type === 'kpi') {
                    var measures = utils_1.buildKPIMeasures(payload_1);
                    if (measures.every(function (m) { return state.measureAxes.some(function (s) { return utils_1.compareAxisWithField(s, m); }); })) {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.removeMeasureAxes, payload: measures }));
                    }
                    else {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.addMeasureAxes, payload: measures.filter(function (m) { return !state.measureAxes.some(function (s) { return utils_1.compareAxisWithField(s, m); }); }) }));
                    }
                }
                    ...
                    if (state.columnAxes.some(function (s) { return utils_1.compareAxisWithField(s, payload_1); })) {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.removeColumnAxis }));
                    }
                    else if (state.rowAxes.some(function (s) { return utils_1.compareAxisWithField(s, payload_1); })) {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.removeRowAxis }));
                    }
                    else if ((state.columnAxes && state.columnAxes.length) && (!state.rowAxes || !state.rowAxes.length)) {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.addRowAxis }));
                    }
                    else {
                        return exports.configuratorReducer(state, tslib_1.__assign({}, action, { type: PIVOT_CONFIGURATOR_ACTION.addColumnAxis }));
                    }
                }
            }
            break;
        ...
        case PIVOT_CONFIGURATOR_ACTION.removeColumnAxis: {
            newColumns = (state.columnAxes || []).filter(function (s) { return !utils_1.compareAxisWithField(s, action.payload); }).slice();
            utils_1.filterField(newColumns, action.payload);
            break;
        }
        ...
        case PIVOT_CONFIGURATOR_ACTION.removeRowAxis: {
            newRows = (state.rowAxes || []).filter(function (s) { return !utils_1.compareAxisWithField(s, action.payload); }).slice();
            utils_1.filterField(newRows, action.payload);
            break;
        }
        ...
        case PIVOT_CONFIGURATOR_ACTION.removeMeasureAxis: {
            newMeasures = (state.measureAxes || []).filter(function (s) { return !utils_1.compareAxisWithField(s, action.payload); }).slice();
            break;
        }
        ...
        case PIVOT_CONFIGURATOR_ACTION.removeMeasureAxes: {
            newMeasures = (state.measureAxes || []).filter(function (s) { return !action.payload.some(function (p) { return utils_1.compareAxisWithField(s, p); }); }).slice();
            break;
        }
        case PIVOT_CONFIGURATOR_ACTION.remove: {
            if (state.columnAxes.some(function (s) { return utils_1.compareAxes(s, action.payload); })) {
                newColumns = state.columnAxes.filter(function (s) { return !utils_1.compareAxes(s, action.payload); }).slice();
                utils_1.filterField(newColumns, { uniqueName: action.payload.name });
            }
            if (state.rowAxes.some(function (s) { return utils_1.compareAxes(s, action.payload); })) {
                newRows = state.rowAxes.filter(function (s) { return !utils_1.compareAxes(s, action.payload); }).slice();
                utils_1.filterField(newRows, { uniqueName: action.payload.name });
            }
            if (state.measureAxes.some(function (s) { return utils_1.compareAxes(s, action.payload); })) {
                newMeasures = state.measureAxes.filter(function (s) { return !utils_1.compareAxes(s, action.payload); }).slice();
            }
            break;
        }
        ...
        case PIVOT_CONFIGURATOR_ACTION.drop:
            if (state.dragItem && state.dropZone) {
                var currentColumn = state.columnAxes.find(function (s) { return utils_1.compareAxes(s, action.payload); });
                var currentRow = state.rowAxes.find(function (s) { return utils_1.compareAxes(s, action.payload); });
                var currentMeasure = state.measureAxes.find(function (s) { return utils_1.compareAxes(s, action.payload); });
                var current = void 0;
                if (currentColumn) {
                ...
                switch (state.dropZone) {
                    case 'columnAxes': {
                        ...
                        utils_1.insertAxis(newColumns, current, state);
                        break;
                    }
                    case 'rowAxes': {
                        newRows = newRows || state.rowAxes.slice();
                        utils_1.insertAxis(newRows, current, state);
                        break;
                    }
                    case 'measureAxes': {
                        newMeasures = newMeasures || state.measureAxes.slice();
                        utils_1.insertAxis(newMeasures, current, state);
                        break;
                    }
                    default:
                        break;
